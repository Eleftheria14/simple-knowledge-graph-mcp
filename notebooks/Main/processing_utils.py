...
class NotebookDocumentProcessor:
    ...
    async def process_single_document(self, doc_status: DocumentStatus) -> bool:
        ...
        try:
            ...
            print(f"   🔍 Step 1: Loading and processing PDF...")
            try:
            ...
            # Step 2: Analysis
            print(f"   🔬 Step 2: Analyzing document content...")
            try:
                ...
                print(f"   🔧 This is a known compatibility issue - using simple fallback")
                ...
                print(f"   ✅ Using simplified document representation")
                all_entities = entities
            
            # Step 3: Knowledge graph storage
            print(f"   🕸️  Step 3: Storing in knowledge graph...")
            try:
                if self.graphiti_engine:
                    ...
                    print(f"   ✅ Stored in Neo4j successfully")
                else:
                    success = True
                    print(f"   ⚠️  Graphiti not available, skipping persistence")
            except Exception as e:
                print(f"   ❌ Knowledge graph storage failed: {e}")
                print(f"   🔍 Error details: {e}")
                # Don't fail completely - mark as success but log the issue
                success = True
                print(f"   ⚠️  Continuing without knowledge graph storage")
    ...
    def print_next_steps(self):
        ...
        print(f"")
        print(f"2. 🔌 Connect to Claude Desktop:")
        print(f'   Add to ~/.config/claude-desktop/config.json:')
        print(f'   "{self.project_name}": {{')
        print(f'     "command": "graphrag-mcp",')
        print(f'     "args": ["serve", "{self.project_name}", "--transport", "stdio"]')
        print(f'   }}')
        print(f"")
        print(f"3. 💬 Try these queries in Claude:")
        print(f'   - "Ask knowledge graph: What are the main themes?"')
        print(f'   - "Get facts with citations about [topic]"')
        print(f'   - "Generate bibliography in APA style"')
