...
# This script now starts the integrated DocsGPT + Knowledge Graph system
# For backwards compatibility, we redirect to the new integrated startup script

echo "🔄 Redirecting to integrated system startup script..."
echo ""

# Check if the integrated startup script exists
if [ -f "$(dirname "$0")/start_integrated_docsgpt.sh" ]; then
    # Run the integrated startup script
    "$(dirname "$0")/start_integrated_docsgpt.sh"
    exit $?
fi
